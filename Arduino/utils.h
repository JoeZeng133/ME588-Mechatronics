...
// public:
    ...
    if(range != nullptr){
      ...
      if(deadBand != nullptr){
        this->range[1] -= this->deadBand[1];
        this->range[0] -= this->deadBand[0];
      }
    }
...
public:
  ...
  void init(float Kp = 0.6, float Ki = 10, float Kd = 0.0002, int flags = 3, int *deadBand = nullptr){
    // filt = new seqFilter<long>(2, 0);
    idx = 0;
    lockinter = 0;
    memset(timeStamp, 0, 4 * sizeof(long));
    //Serial.println("Initialize motor PID controll");
    pid = new PID(Kp, Ki, Kd, flags, nullptr, range);
...
//    Serial.println(pid->u);
  ...
  void drive(float const &setpoint){
   ...
   // Serial.print(" ");
    ...
    updateSpeed();
    //which happends when there is long time no driving
    if(interv > 0.02) interv = 0.01;
    ...
    if(setdir == dir){
      ...
      // analogWrite(pinPWM, 255.0 - output);

      if(whichMotor == 1){
        state.drive_rpm[whichMotor + 1] = 90;
        analogWrite(pinPWM, 255 - output);
      }else
      {
        // if(output == 255){
        //         state.ypr[0] = state.ypr[1] = state.ypr[2] = 100;
        //         // analogWrite(pinPWM, 0);
        //         analogWrite(pinPWM, 255);
        // }else{
        //         state.ypr[0] = state.ypr[1] = state.ypr[2] = 120;
        //         analogWrite(pinPWM, 255 - output);
        // }
        analogWrite(pinPWM, 255 - output);
        state.drive_rpm[whichMotor + 1] = 255 - output;
...
//	pinMode(pinCh2, INPUT);
  ...
  void init(float Kp = 5 , float Ki = 10, float Kd = 0, int flags = 6, int *deadBand = nullptr){
//  	N = 0;
    idx = 0;
    lockinter = 0;
    memset(timeStamp, 0, 4 * sizeof(long));
    // Serial.println("Initialize motor PID controll");
    range[1] = range[1] - dBand[1];
    range[0] = range[0] - dBand[1];

    pid = new PID(Kp, Ki, Kd, 6, nullptr, range);
...
//    attachInterrupt(digitalPinToInterrupt(pinCh2), isr[whichMotor], CHANGE);
  ...
  void drive(float const &setpoint){
    ...
    if(setdir == dir){
      //setpoint direction and current direction is the same
      pid->update(abs(setpoint), speed_motor, interv, output);
      // if(output > 5) setdir = 1;
      // if(output < -5) setdir = 0;

      // if(setpoint > 0) output += dBand[1];
      // if(setpoint < 0) output -= dBand[1];

      // digitalWrite(pinIn1, setdir);
      // digitalWrite(pinIn2, !setdir);
      analogWrite(pinEna, output + dBand[1]);
      // analogWrite(pinEna, 200);
    }
