...
#	cameraItr,_ = cameraInit()
	...
	print("calibrate obstacle color")
	low, high = adjustHSV(cameraItr)
	f.write(str(low[0]) + ' ' + str(low[1]) + ' ' + str(low[2]) + ' ')
	f.write(str(high[0]) + ' ' + str(high[1]) + ' ' + str(high[2]))
...
def getVal():
	...
	lowobs = np.array([int(s[12]), int(s[13]), int(s[14])])
	highobs = np.array([int(s[15]), int(s[16]), int(s[17])])
	f.close()
...
def main():
	...
	lowBall, highBall, lowCon, highCon, lowobs, highobs = getVal()
...
#	highCon = np.array([115 , 255 , 255])
	...
	obsDec = detector(lowobs, highobs, circleContour(), cameraItr)
	...
	while(ballHave < 3 and time.time() - starttime < 45):
		if findAround(ballDec, chooseTennisBall):
			follow(ballDec, obsDec, ballBehave, chooseTennisBall)
		
	logicState = 1
	print("Find container _____________________________________")
	if findAround(conDec, chooseCon):
		follow(conDec, obsDec, conBehave, chooseCon)
